...
async function formatChatBotAuthCheck(config: AppConfig): Promise<string[]> {
  ...
  const keyPath = configuredPath ? resolveKeyFilePath(configuredPath) : undefined;
  if (!keyPath) {
...
async function probeChatBotAccessToken(config: AppConfig): Promise<string[]> {
  try {
    ...
    const clientEmail =
      "email" in client && typeof client.email === "string" ? client.email : "(unknown)";
    ...
    return ["- Chat token probe: FAILED", `- Error: ${formatProbeError(error)}`];
  }
}
...
async function readServiceAccountMetadata(keyPath: string): Promise<ServiceAccountMetadata> {
  try {
