...
export function createApp(
  ...
  app.post("/google-chat/events", async (req, res, next) => {
    try {
      const response = await resolvedDeps.handleChatEvent(config, storage, req.body);
      res.json(response);
    } catch (error) {
      next(error);
    }
  ...
  app.get("/auth/google/start", async (req, res, next) => {
    try {
      ...
      res.redirect(await resolvedDeps.buildAuthUrl(config, storage, chatUserId));
    } catch (error) {
      next(error);
    }
  });
  ...
  app.get("/auth/google/callback", async (req, res, next) => {
    try {
      ...
      const email = await resolvedDeps.completeOAuth(config, storage, code, state);
      res.type("html").send(`
        <!doctype html>
        <html lang="ru">
